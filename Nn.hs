...
-- @since 0.1.0
new :: [Int] -> IO Network
new n = newGen n <$> getSeed
  where getSeed = do {putStr "seed: "; read <$> getLine}

newWithSeed = flip newGen
...
--
-- @since 0.1.0
newGen :: [Int] -> Integer -> Network
newGen n seed = (sigmoidLayer <$> init wss) ++ [outputLayer (last wss)]
 where
  rest                 = init n
  hiddenIcsNcs         = zip ((+ 1) <$> rest) (tail rest)
  (outputIc, outputNc) = (snd (last hiddenIcsNcs) + 1, last n)
  rs                   = (map range . iterate random) seed
  (hidden, rs')        = foldl
    ( \(wss', rr') (ic, nc) ->
